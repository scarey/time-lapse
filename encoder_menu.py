# tweaked version of https://github.com/sgall17a/encodermenu/blob/main/encoder_menu.py
...
encoder = RotaryIRQ(pin_num_clk=13,
                    pin_num_dt=5,
                    min_val=0,
                    max_val=100,
                    reverse=False,
...
def display(text1, text2, row_start=0):
    oled.fill(0)
    oled.text(text1, 0, row_start)
    oled.text(text2, 0, row_start + 30)
    oled.show()
...
def mainloop():
    ...
    while menu_data.get('finished', 'No') == 'No':
        await step()
...
class Menu:
    "Show a menu on a tiny display by turning a rotary encoder"
